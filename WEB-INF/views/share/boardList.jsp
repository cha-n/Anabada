...
<script type="text/javascript">
	...
	function _bookmark(boardSeq) {
		...
		if("${log}"=='') {
			alert("로그인이 필요합니다.");
			location.href = "${contextPath}/member/loginForm?action=/share/boardList";
		}
	...
	function goBoardWrite(log){
		if(log==true)
			...
			location.href = "${contextPath}/share/boardWrite";
		else {
			alert("로그인이 필요합니다.");
			location.href = "${contextPath}/member/loginForm?action=/share/boardWrite";
		}
	}
	...
	function getBoardList(currentPageNo){
		...
		$.ajax({	
			...
			url		:"${contextPath}/share/getBoardList?"+"searchType="+$("select option:selected").val()+ "&keyword="+encodeURIComponent($('#keywordInput').val()),
