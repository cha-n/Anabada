...
<script>
	function contentClick(review_NO) { //수정버튼,삭제버튼 열고 닫는 기능
		var buttonOpenCheck=$("#buttonOpenCheck").val();
		if(buttonOpenCheck!=review_NO) {
			var span=document.getElementById("span"+review_NO);
			span.innerHTML+='<input type="button" value="수정" class="btn btn-primary py-1 px-3" onclick="modReview('+review_NO+')">  ';
			span.innerHTML+='<input type="button" value="삭제" class="btn btn-primary py-1 px-3" onclick="delReview('+review_NO+')">';
			
			if(buttonOpenCheck!="0") {
				document.getElementById("span"+buttonOpenCheck).innerHTML='';
			}
			$("#buttonOpenCheck").val(review_NO);
		}
		else {
			var span=document.getElementById("span"+review_NO);
			span.innerHTML='';
			$("#buttonOpenCheck").val("0");
		}
	}
	
	function modReview(review_NO) {
		var form = document.createElement("form");
		form.setAttribute("method", "post");
		form.setAttribute("action", "${contextPath}/seller/modReview");
		
	    var reviewNOInput = document.createElement("input");
	    reviewNOInput.setAttribute("type","hidden");
	    reviewNOInput.setAttribute("name","review_NO");
	    reviewNOInput.setAttribute("value", review_NO);
	...
	function delReview(review_NO) {
		if(window.confirm("후기를 삭제하시겠습니까?")) {
			var form = document.createElement("form");
			form.setAttribute("method", "post");
			form.setAttribute("action", "${contextPath}/seller/deleteReview");
  			...
  			<div class="col-md-12 ftco-animate">
				  ...
				  <c:choose>
				  	<c:when test="${empty reviewList }">
						<h4 style="text-align:center;padding:2rem 0 2rem 0;">작성한 후기가 없습니다.</h4>
				  	</c:when>
				  	<c:otherwise>
					  <table style="min-width:1000px ! important;width:100%;">
					  	<c:forEach var="review" items="${reviewList}" varStatus="num">
					  		<tr class="line1">
					  			<td style="padding:20px 10px;color:black;">
					  				<b>${review.articleVO.title }&nbsp;&nbsp;</b>
					  				<img src="${contextPath }/resources/image/star.png" width="15" height="15">
					  				<c:if test="${review.star==1 }">
					  					<img src="${contextPath }/resources/image/star2.png" width="15" height="15">
					  					<img src="${contextPath }/resources/image/star2.png" width="15" height="15">
					  					<img src="${contextPath }/resources/image/star2.png" width="15" height="15">
					  					<img src="${contextPath }/resources/image/star2.png" width="15" height="15">
					  				</c:if>
					  				...
					  				<c:if test="${review.star==4 }">
					  					...
					  					<img src="${contextPath }/resources/image/star.png" class="profile" width="15" height="15">
					  					<img src="${contextPath }/resources/image/star.png" class="profile" width="15" height="15">
					  					<img src="${contextPath }/resources/image/star2.png" class="profile" width="15" height="15">
					  				</c:if>
					  			...
					  			<td style="padding:20px 10px;text-align:end;">${review.writedate }
					  				
					  			</td>
					  		</tr>
					  		<tr class="line1">
					  			<td colspan="2" style="cursor:pointer;padding:30px 10px;border:0;" onclick="contentClick(${review.review_NO})">
					  				<span>${review.content }</span>
					  				<input type="hidden" id="buttonOpenCheck" value="0" >
					  			</td>
					  		</tr>
					  		<tr class="line2">
					  			<td style="padding:10px 10px;"> </td>
					  			<td style="padding:10px 10px;text-align:end;"><span id="span${review.review_NO }"></span></td>
					  		</tr>
					  	</c:forEach>
					  </table>
					 </c:otherwise>
