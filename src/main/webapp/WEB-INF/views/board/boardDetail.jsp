...
/** 예슬이 짠 코드    */
   $(function(){
      $('#content').bind('focus',function(event){
         if($('#log').val()=='') { //로그인이 안되어 있다면
            ...
            alert("로그인이 필요합니다.");
            location.href='${contextPath}/member/loginForm'+'?action=/board/boardDetail?boardSeq='+post_NO;
         }
      });
   });
   ...
   function _write(num) { //댓글,대댓글 작성
      ...
      if(num==0) { //댓글
         ...
         $.ajax({
            type:"post",
            async:false,
            url:"${contextPath}/board/addComment",
            data:{
               seq:1,
               post_NO:post_NO,
               n_name:n_name,
            ...
            success:function (data){
               if(data=='ok'){
                  alert("댓글을 작성했습니다.");
                  location.href='${contextPath}/board/boardDetail?boardSeq='+post_NO;
               }
            },
            error:function(data){
   ...
   function _delete(comment_NO) { //댓글 삭제
      ...
      if(confirm("댓글을 삭제하시겠습니까?")==true) {
         $.ajax({
            type:"post",
            async:false,
            url:"${contextPath}/board/deleteComment",
   ...
   function _bookmark() {
      ...
      if(image.alt=="uncheck") { //북마크 해지된 상태일 때
         if("${log}"=='') { //로그인이 안되어 있는 상태일 때
            alert("로그인이 필요합니다.");
            location.href = "${contextPath}/member/loginForm?action=/board/boardDetail?boardSeq=" + boardSeq;
         }
   ...
   function changeSell_YN(sell_YN) { //판매여부  전환
      ...
      var boardSeq = $("#board_seq").val(); //글번호
        $.ajax({
           type:"post",
           async:false,
           url:"${contextPath}/board/updateSell_YN",
           ...
           success:function (data,textStatus){
              if(data=='ok'){
                 if(sell_YN=='N') {
                    alert("판매완료로 전환했습니다.");
                    location.href='${contextPath}/board/boardDetail?boardSeq='+boardSeq;
                 }
                 else {
                    alert("판매중으로 전환했습니다.");
                    location.href='${contextPath}/board/boardDetail?boardSeq='+boardSeq;
   ...
   function goBoardUpdate() {
      ...
      location.href = "${contextPath}/board/boardUpdate?boardSeq=" + boardSeq;
   }
   /** 게시판 - 답글 페이지 이동 */
   function goBoardReply() {
      var boardSeq = $("#board_seq").val();
      if("${log}"==true) {
         location.href = "${contextPath}/board/boardReply?boardSeq=" + boardSeq;
      }
      else {
         alert("로그인이 필요합니다.");
         location.href = "${contextPath}/member/loginForm?action=/board/boardReply?boardSeq=" + boardSeq;
      }
   }
   /** 게시판 - 첨부파일 다운로드 */
   ...
   function getBoardDetail(boardSeq) {
      ...
      if (boardSeq != "") {
         ...
         $.ajax({
            url : "${contextPath }/board/getBoardDetail?boardSeq=" + boardSeq,
   ...
   function getBoardDetailCallback(obj) {
      ...
      if (obj != null) {
         ...
         if("${member.n_name}"==boardWriter) {
             if(sell_yn=="판매중") {
                str += "<button type='button' onclick='changeSell_YN(0)' style='margin-left:3%;'>판매완료로 전환</button>";
             }
             else {
                str += "<button type='button' onclick='changeSell_YN(1)' style='margin-left:3%;'>판매중으로 전환</button>";
             }
   ...
   function deleteBoard() {
      ...
      if (yn) {
         $.ajax({
            url : "${contextPath}/board/deleteBoard",
	...
	<div class="col text-center">
		<div class="form-group">
        	<c:if test="${action != 'book' and active !='mylist'}">
                ...
                onclick="javascript:goBoardList();">글 목록</button>
            </c:if>
