   ...
   function _pay(obj) {
      if($("#addressDetail").val()=="") {
         ...
         $.ajax({
            ...
            data:$("#deliveryForm,#deliveryForm2").serialize(),
			...
			<div class="row justify-content-center">
				<div class="col-xl-7 ftco-animate">
					<form name="deliveryForm" id="deliveryForm" method="post" action="${contextPath }/order/completed" class="billing-form">
						...
						<h3 class="mb-4 billing-heading">주문상세</h3>
					<table style="width:100%;">
				  	<tr bgcolor="#82ae46" style="color:white;padding:3%;">
				  		<td width="30%" colspan="2" style="padding:1.5%;">상품 정보</td>
				  		<td width="15%">판매가격</td>
				  		<td width="10%">수량</td>
				  		<td width="15%">주문금액</td>
				  	</tr>
				  	<c:set var="resultPrice" value="0" />
				  		<c:forEach  var="cart" items="${cartList }" varStatus="num">
							<tr>
						  		...
						  		<img src="${contextPath}/download.do?articleNO=${cart.articleNO}&imageFileName=${cart.articleVO.imageFileName}" id="preview${num.count}" width="50" height="50"/></td>
						  		<td style="text-align:left;">
						  			<span onclick="view_Detail('${cart.articleNO}')" style="cursor:pointer;">${cart.articleVO.title }</span>
						  		...
						  		<td>${cart.articleVO.price * cart.quantity }원
					  	...
					  	<tr>
							<td>
					  		<input type="hidden" id="len" name="len" value="${fn:length(cartList)}"/></td>
						</tr>
				  </table>
				  <br>
						<div class="row align-items-end">
							<div class="col-md-6">
								<div class="form-group">
									...
									<input type="text" class="form-control" value="${member.name }" style="color:gray !important;">
								</div>
							</div>
							<div class="w-100"></div>
							<div class="col-md-9">
								<div class="form-group">
									...
									<p><input type="text" class="form-control" id="_address" value="${member.address }" disabled style="width:75%;display:inline-block;color:gray !important;">
									<input type="hidden" name="address" id="address" value="${member.address }" >
									<input type="button" value="주소 찾기" onclick="_findAddress()" class="btn btn-black px-3 py-2" style="margin-left:1%;"></p>
								</div>
							</div>
							<div class="w-100"></div>
							<div class="col-md-12">
								<div class="form-group">
									...
									<input type="text" class="form-control" name="addressDetail" id="addressDetail" placeholder="상세 주소를 입력하세요." style="color:gray !important;">
									<b id="a_warning"></b>
								</div>
							</div>
							<div class="w-100"></div>
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<div style="float: left; width: 25%;margin-right:10px;">
										<input type="text" class="form-control" id="_phone1" size="3" maxlength="3" style="color:gray !important;" >
									</div>
									<div style="float: left; width: 29%;margin-right:10px;">
										<input type="text" class="form-control" id="_phone2" size="4" maxlength="4" style="color:gray !important;" >
									</div>
									<div style="float: left; width: 29%;">
										<input type="text" class="form-control" id="_phone3" size="4" maxlength="4" style="color:gray !important;" >
									</div>
					...
					<div class="row mt-5 pt-3">
						<div class="col-md-12 d-flex mb-5">
							<div class="cart-detail cart-total p-3 p-md-4">
								...
								<p class="d-flex">
									<span>금액</span> <span>${resultPrice}원</span>
								</p>
								<p class="d-flex">
									<span>배송료</span> <span>0원</span>
								</p>
								<p class="d-flex">
									<span>할인</span> <span>0원</span>
								</p>
								<hr>
								<p class="d-flex total-price">
									<span>결제 금액</span> <span>${resultPrice }원</span>

								</p>
							</div>
						</div>
						<!-- 결제 방법 -->
						<form name="deliveryForm2" id="deliveryForm2" method="post" action="${contextPath }/order/completed">
							<input type="hidden" id="pay_method" name="pay_method">
							<div class="col-md-12">
								<div style="width: 445px;" class="cart-detail p-3 p-md-4">
									<h3 class="billing-heading mb-4"><font size="3.9em">결제 방법</font></h3>
									<div class="form-group">
										<div class="col-md-12">
											<div class="radio">
												<label><input type="radio" name="optradio" class="mr-2" onclick="open_Deposit()"> 무통장 입금</label>
											</div>
										</div>
										<div id="depositDiv" style="display: none; width: 70%; margin: 0 auto 0 auto;">
											<table cellpadding="3" border="1">
												<tr>
													<td width="500">입금할 은행</td>
													<td><select id="_bank1" name="_bank1">
															<option value="농협" selected>농협</option>
															<option value="국민은행">국민은행</option>
															<option value="신한은행">신한은행</option>
															<option value="우리은행">우리은행</option>
															<option value="하나은행">하나은행</option>
													</select> <span id="depositSpan"></span></td>
												</tr>
												<tr>
													<td width="500">입금자 명</td>
													<td><span id="depositNameSpan"></span></td>
												</tr>
											</table>
										</div>
									</div>
									<div class="form-group">
										<div class="col-md-12">
											<div class="radio">
												<label><input type="radio" name="optradio" class="mr-2" onclick="open_Card()"> 신용카드</label>
											</div>
										</div>
										<div id="cardDiv" style="display: none; width: 70%; margin: 0 auto 0 auto;">
											<table cellpadding="3" border="1">
												<tr>
													<td width="500">카드 종류</td>
													<td><select id="_bank2" name="_bank2">
													...
													</select> <span id="cardSpan"></span></td>
												</tr>
											</table>
										</div>
									</div>
									...
									<input type="button" class="btn btn-primary py-3 px-4" value="결제" onclick="_pay(deliveryForm)">
								</div>
							</div>
						</form>
					</div>
