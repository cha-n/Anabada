...
<script>
	$(document).ready(function() {
		...
		$(".selectDelete_btn").click(function() {
			...
			if (confirm_val) {
				...
				$.ajax({
					url : "${contextPath}/exchange/deleteMyExchangeList?user="+ user, //전송
					type : "post",
					data : {
						chbox : checkArr
					},
					success : function(result) {
						if (result == 1) {
							location.href = "${contextPath}/exchange/myExchangeList?user="+ user; //전송이 이상없으면 새로고침
						} else {
							alert("삭제 실패");
						}
					}
	...
	function goMyFarmerList() {
		var user = $("#board_writer").val(); //글작성자
		location.href = "${contextPath}/board/myFarmerList?user="+user;
	}
	...
	function goBoardDetail(boardSeq) {
		location.href = "${contextPath}/exchange/boardDetail?boardSeq=" + boardSeq+ "&active=mylist";
	}

	function getMyExchangeList(currentPageNo) {
		...
		$.ajax({
			...
			url : "${contextPath}/exchange/getMyExchangeList?user="+user,
	...
	/** 게시판 - 목록 조회  콜백 함수 */
	function getMyExchangeListCallback(obj) {
		...
		if (state == "SUCCESS") {
			...
			if (listLen > 0) {
				...
				for (var a = 0; a < listLen; a++) {
					...
					if( n_name==user){
					str += "<td class='checkBox'><input type='checkbox' name='chBox' class='chBox' value="+boardSeq+" />"+ "</td>";
					}
					//정은이
					str += "<td class='image-prod'><div class='img'><img style='width:100%;' src ='${contextPath}/board/fileDownload?fileNameKey="+encodeURI(fileNameKey)+"&fileName="+encodeURI(fileName)+"&filePath="+encodeURI(filePath)+"'  width='90' height='90'></div></td>";
...
<!-- 			<div class="sidebar-box ftco-animate">
			<div class="tagcloud">
				<a href="#" class="tag-cloud-link" onclick="goFarmerList()">공동구매</a>
				<a href="#" class="tag-cloud-link" onclick="goMyShareList()">나눔</a>
				<a href="#" class="tag-cloud-link" onclick="goMyBoardList()">판매</a>
				<a href="#" class="tag-cloud-link" onclick="getMyExchangeList()">교환</a>
			</div>
			</div>
    			...
    			<div class="col-md-12 ftco-animate">
    				...
    				<span>
 					<input type="button" value="판매" id="boardButton" class="btn btn-primary py-2 px-3" style="background:#82ae46;border:0;" onclick="goMyBoardList()">
  					<input type="button" value="공동구매" id="farmerButton" class="btn btn-primary py-2 px-3" style="background:#9f9f9f;border:0;" onclick="goMyFarmerList()">
  					<input type="button" value="교환" id="exchangeButton" class="btn btn-primary py-2 px-3" style="background:#9f9f9f;border:0;" onclick="getMyExchangeList()">
