...
/** 예슬이 짠 코드    */
	$(function(){
		$('#content').bind('focus',function(event){
			if($('#log').val()=='') { //로그인이 안되어 있다면
				...
				alert("로그인이 필요합니다.");
				location.href='${contextPath}/member/loginForm'+'?action=/exchange/boardDetail?boardSeq='+post_NO;
			}
		});
	});
	...
	function _write(num) { //댓글,대댓글 작성
		...
		if(num==0) { //댓글
			...
			$.ajax({
				type:"post",
				async:false,
				url:"${contextPath}/exchange/addComment",
				data:{
					seq:1,
					post_NO:post_NO,
					n_name:n_name,
				...
				success:function (data){
					if(data=='ok'){
						alert("댓글을 작성했습니다.");
						location.href='${contextPath}/exchange/boardDetail?boardSeq='+post_NO;
					}
				},
				error:function(data){
	...
	function _update(comment_NO) { //댓글 수정
		...
		$.ajax({
			type:"post",
			async:false,
			url:"${contextPath}/exchange/updateComment",
			data:{
	...
	function _delete(comment_NO) { //댓글 삭제
		...
		if(confirm("댓글을 삭제하시겠습니까?")==true) {
			$.ajax({
				type:"post",
				async:false,
				url:"${contextPath}/exchange/deleteComment",
	...
	function _bookmark() {
		...
		if(image.alt=="uncheck") { //북마크 해지된 상태일 때
			if("${log}"=='') { //로그인이 안되어 있는 상태일 때
				alert("로그인이 필요합니다.");
				location.href = "${contextPath}/member/loginForm?action=/exchange/boardDetail?boardSeq=" + boardSeq;
			}
				...
				var n_name="${member.n_name}";
				  $.ajax({
					  type:"post",
					  async:false,
					  url:"${contextPath}/exchange/addExchangeBookmark",
			...
			var n_name="${member.n_name}";
			  $.ajax({
				  type:"post",
				  async:false,
				  url:"${contextPath}/exchange/deleteExchangeBookmark",
				  data: {post_NO : boardSeq,
					     n_name : n_name,
					     category : "교환"
				  },
	...
	function changeSell_YN(sell_YN) { //판매여부  전환
		...
		var boardSeq = $("#board_seq").val(); //글번호
		  $.ajax({
			  type:"post",
			  async:false,
			  url:"${contextPath}/exchange/updateSell_YN",
	...
	function goBoardUpdate() {
		...
		location.href = "${contextPath}/exchange/boardUpdate?boardSeq=" + boardSeq;
	}
	...
	function goBoardReply() {
		var boardSeq = $("#board_seq").val();
		if("${log}"==true) {
			location.href = "${contextPath}/exchange/boardReply?boardSeq=" + boardSeq;
		}
		else {
			alert("로그인이 필요합니다.");
			location.href = "${contextPath}/member/loginForm?action=/exchange/boardReply?boardSeq=" + boardSeq;
		}
	}
	...
	function getBoardDetailCallback(obj) {
		...
		if (obj != null) {
			...
			if (filesLen > 0) {
					...
					for (var a = 0; a < filesLen; a++) {
						...
						console.log("fileName : " + fileName);
	 						...
	 						//이미지 넣을 부분   	 													    				    	    
							str += "<img src ='${contextPath}/board/fileDownload?fileNameKey="+encodeURI(fileNameKey)+"&fileName="+encodeURI(fileName)+"&filePath="+encodeURI(filePath)+"' height='350' width='100%' class='img-fluid' alt='Colorlib Template'></a>";
	 						}
			}	
			...
			if("${member.n_name}"==boardWriter) {
 				if(sell_yn=="교환중") {
 					str += "<button type='button' onclick='changeSell_YN(0)' style='margin-left:3%;'>교환완료로 전환</button>";
 				}
 				else {
 					str += "<button type='button' onclick='changeSell_YN(1)' style='margin-left:3%;'>교환중으로 전환</button>";
 				}
	...
	function deleteBoard() {
		...
		if (yn) {
			...
			$.ajax({
				...
				url : "${contextPath}/exchange/deleteBoard",
 	...
 	<div class="col text-center">
		<div class="form-group">
        	<c:if test="${action != 'book' and active !='mylist'}">
            	<button type="button" class="btn btn-white py-3 px-5"
