...
import com.spring.mysqltest.exchange.form.CommonForm;
...
public class BoardServiceImpl implements BoardService {
	...
	public List<BoardFileForm> getBoardFileInfo(BoardForm boardForm) throws Exception {
		...
		// 파일이 저장될 Path 설정
		String filePath = "/home/anabada/images/exchange";
	...
	public BoardDto deleteBoard(BoardForm boardForm) throws Exception {
        ...
        File f=new File("/home/anabada/images/exchange/"+fileNameKey);
        if(f.exists()) {
	    	f.delete(); //삭제
	    }
	...
	public ResultUtil getMyExchangeList(BoardForm boardForm, MemberVO member) throws Exception {
		...
		List<BoardDto> myboardList = null;
		 ...
		 if( n_name.equals(user)) { 
			 //본인이 쓴 글 보기 		 
			totalCount = boardDao.getMyExchangeCnt(n_name);
			...
			myboardList = boardDao.getMyExchangeList(member);
		
		} else { 
			//판매자 글정보 보기 
			totalCount = boardDao.getSellerExchangeCnt(user);
			...
			boardForm.setOffset(commonDto.getOffset());
			myboardList = boardDao.getSellerExchangeList(boardForm); 
		}
	...
	public void deleteMyExchangeList(BoardForm boardForm) throws Exception {
		boardDao.deleteMyExchange(boardForm);
	}
	...
	@Override
	public String checkExchangeBookmark(Map<String, Object> bookMap) throws Exception { // 교환 카테고리에서 해당 글 북마크 여부 확인
		return boardDao.checkExchangeBookmark(bookMap);
	}

	@Override
	public boolean addExchangeBookmark(Map<String, Object> bookMap) throws Exception { // 교환 카테고리에서 해당 글 북마크 설정
		boardDao.addExchangeBookmark(bookMap);
		boolean result = true;
		return result;
	}

	@Override
	public boolean deleteExchangeBookmark(Map<String, Object> bookMap) throws Exception { // 교환 카테고리에서 해당 글 북마크 설정
		boardDao.deleteExchangeBookmark(bookMap);
		boolean result = true;
		return result;
	}

	@Override
	public List<BookmarkVO> listExchangeBookmark(Map<String, Object> bookMap) throws Exception { // 교환 카테고리 중 회원이 추가해놓은 북마크
		List<BookmarkVO> bookmarkList = null;
		bookmarkList = boardDao.listExchangeBookmark(bookMap);
		return bookmarkList;
	}
